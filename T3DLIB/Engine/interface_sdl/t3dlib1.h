...
typedef struct BOB_TYP
{
    ...
    SURFACE images[MAX_BOB_FRAMES]; // the bitmap images DD surfaces
...
int Draw_BOB(BOB_PTR bob, SURFACE dest);
int Draw_Scaled_BOB(BOB_PTR bob, int swidth, int sheight,SURFACE dest);
int Draw_BOB16(BOB_PTR bob, SURFACE dest);
int Draw_Scaled_BOB16(BOB_PTR bob, int swidth, int sheight,SURFACE dest);
...
inline void Mem_Set_WORD(void *dest, USHORT data, int count)
{
    ...
    //wmemset((wchar_t*)dest, data,  count);
