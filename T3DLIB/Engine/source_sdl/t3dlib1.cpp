...

{
    ...
    // are created as offscreen surfaces in VRAM as the default
    ...
    for (index=0; index<bob->num_frames; index++)
    {
        ...
        bob->images[index] = create_surface(width, height);
        ...
        set_color_key(bob->images[index], color_key_value);
...
int Draw_BOB(BOB_PTR bob,               // bob to draw
             ...
             SURFACE dest) // surface to draw the bob on
{
    // draw a bob at the x,y defined in the BOB
    // on the destination surface defined in dest

    SURFACE_RECT dest_rect,   // the destination rectangle
    ...
    dest_rect.x  = bob->x;
    dest_rect.y  = bob->y;
    dest_rect.w  = bob->width;
    dest_rect.h  = bob->height;

    // fill in the source rect
    source_rect.x  = 0;
    source_rect.y  = 0;
    source_rect.w  = bob->width;
    source_rect.h  = bob->height;

    // blt to destination surface
    blit_surface(bob->images[bob->curr_frame], &source_rect, dest, &dest_rect);
                    ...
                    SURFACE dest) // surface to draw the bob on)
{
    ...
    // blt to destination surface
    blit_surface_scaled_8b(bob->images[bob->curr_frame], &source_rect, dest, &dest_rect);
                      ...
                      SURFACE dest) // surface to draw the bob on)
{
    ...
    // blt to destination surface
    blit_surface_scaled_16b(bob->images[bob->curr_frame], &source_rect, dest, &dest_rect);
                   ...
                   // cx,cy are absolute coords
{
    ...
    source_ptr = (UCHAR *)bitmap->surface->pixels + cy*bitmap->surface->w+cx;
    ...
    /*(bob->images[frame])->Lock(NULL,
        ...
        NULL);*/
    lock_surface(bob->images[frame]);

    // assign a pointer to the memory surface for manipulation
    //dest_ptr = (UCHAR *)ddsd.lpSurface;
    dest_ptr = (UCHAR *)bob->images[frame]->pixels;
    ...
    for (int index_y=0; index_y<bob->height; index_y++)
    {
        ...
        dest_ptr   += (bob->images[frame]->pitch);
        source_ptr += bitmap->surface->w;
    } // end for index_y

    // unlock the surface
    //(bob->images[frame])->Unlock(NULL);
    unlock_surface(bob->images[frame]);
                     ...
                     // cx,cy are absolute coords
{
    ...
    source_ptr = (USHORT *)bitmap->surface->pixels + cy*bitmap->surface->w+cx;
    ...
    dest_ptr = (USHORT *)bob->images[frame]->pixels;
    ...
    for (int index_y=0; index_y<bob->height; index_y++)
    {
        ...
        //source_ptr += bitmap->bitmapinfoheader.biWidth;
        dest_ptr   += (bob->images[frame]->pitch >> 1);
        source_ptr += bitmap->surface->w;
    } // end for index_y
...
int Load_Image_Bitmap(BITMAP_IMAGE_PTR image,  // bitmap image to load with data
                      ...
                      // cx,cy are absolute coords
{
    load_image_bitmap(image, bitmap, cx, cy, mode); return 1;
...
int Load_Image_Bitmap16(BITMAP_IMAGE_PTR image,  // bitmap image to load with data
                        ...
                        // cx,cy are absolute coords
{
    load_image_bitmap16(image, bitmap, cx, cy, mode); return 1;
...
{
    load_bitmap(bitmap, filename); return 1;
    //load_bitmap_file(bitmap, filename); return 1;
