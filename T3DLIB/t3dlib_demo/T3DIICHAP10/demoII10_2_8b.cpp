// DEMOII10_2_8b.CPP - Terrain simualtion with clipping, 8-bit version
// READ THIS!
// To compile make sure to include:
// the C++ source modules T3DLIB1-8.CPP and the headers T3DLIB1-8.H
...
int Game_Init(void *parms)
{
    ...
    car_sound_id = load_wav("CARIDLE.WAV");

    // start the sounds
    play_sound(wind_sound_id, -1);
    play_sound(car_sound_id, -1);

    sound_set_volume(car_sound_id, 70);
    sound_set_volume(wind_sound_id, 100);

    // load in the cockpit image
    Load_Bitmap_File(&bitmap8bit, "lego01_256.BMP");
    Create_BOB(&cockpit, 0,0,800,600,1, BOB_ATTR_VISIBLE | BOB_ATTR_SINGLE_FRAME, 0, 0, 8);
    Load_Frame_BOB(&cockpit, &bitmap8bit,0,0,0,BITMAP_EXTRACT_MODE_ABS);
    ...
    Generate_Terrain_OBJECT4DV2(&obj_terrain,            // pointer to object
                                ...
                                "earthheightmap01.bmp",  // filename of height bitmap encoded in 256 colors
                                "earthcolormap01_256.bmp",   // filename of texture map
...
int Game_Main(void *parms)
{
    ...
    while(main_loop)
    {
        ...
        // draw the sky
        draw_rectangle(0,0, WINDOW_WIDTH, WINDOW_HEIGHT*.38, rgblookup[RGB16Bit565(38,168,252)]);

        // draw the ground
        draw_rectangle(0,WINDOW_HEIGHT*.38, WINDOW_WIDTH, WINDOW_HEIGHT, rgblookup[RGB16Bit565(85,85,255)]);
        ...
        if (help_mode==1)
        {
            ...
            draw_text("<X>..............Toggle X axis clipping.", 0, text_y+=12, RGB(255,255,255));
            draw_text("<Y>..............Toggle Y axis clipping.", 0, text_y+=12, RGB(255,255,255));
            draw_text("<Z>..............Toggle Z axis clipping.", 0, text_y+=12, RGB(255,255,255));
