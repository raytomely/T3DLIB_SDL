// DEMOII8_5_8b.CPP - flat shading demo (8-bit version)
// READ THIS!
...
int Game_Main(void *parms)
{
    ...
    while(main_loop)
    {
        ...
        // perform lighting
        if (lighting_mode==1)
           Light_OBJECT4DV1_World(&obj_player, &cam, lights, 4);
        ...
        for (index = 0; index < NUM_TANKS; index++)
        {
            ...
            if (!Cull_OBJECT4DV1(&obj_tank, &cam, CULL_OBJECT_XYZ_PLANES))
            {
               ...
               // perform lighting
               if (lighting_mode==1)
                  Light_OBJECT4DV1_World(&obj_tank, &cam, lights, 4);
        ...
        for (index = 0; index < NUM_TOWERS; index++)
        {
            ...
            if (!Cull_OBJECT4DV1(&obj_tower, &cam, CULL_OBJECT_XYZ_PLANES))
            {
               ...
               // perform lighting
               if (lighting_mode==1)
                  Light_OBJECT4DV1_World(&obj_tower, &cam, lights, 4);
        ...
        for (int index_x = 0; index_x < NUM_POINTS_X; index_x++)
            for (int index_z = 0; index_z < NUM_POINTS_Z; index_z++)
            {
                ...
                if (!Cull_OBJECT4DV1(&obj_marker, &cam, CULL_OBJECT_XYZ_PLANES))
                {
                   ...
                   // perform lighting
                   if (lighting_mode==1)
                      Light_OBJECT4DV1_World(&obj_marker, &cam, lights, 4);
        ...
        // render the object
        if (wireframe_mode  == 1)
           Draw_RENDERLIST4DV1_Wire(&rend_list, back_buffer, back_lpitch);
        else
