// DEMOII12_11.CPP - Mipmapping demo
// READ THIS!
// To compile make sure to include:
// the C++ source modules T3DLIB1-10.CPP and the headers T3DLIB1-0.H
...
char *object_filenames[NUM_OBJECTS] = {
                                        "cube_constant_textured_01.cob",
                                        "cube_constant_textured_02.cob",
                                        "fire_constant_cube01.cob",
                                        "borg_cube_constant01.cob",
...
int Game_Main(void *parms)
{
    ...
    static float mipdistance    = 2500;
    static int mip_mode       = 1;
    ...
    while(main_loop)
    {
        ...
        if (keystate[KBD_1])
        {
           if ((mipdistance-=10) < 0) mipdistance = 0;
           // wait, so keyboard doesn't bounce
           sleep_ms(1);
        } // end if
        ...
        if (wireframe_mode  == 1)
        {
           ...
           rc.attr         = RENDER_ATTR_INVZBUFFER
                             | (mip_mode==1 ? RENDER_ATTR_MIPMAP   : 0)
        ...
        #if 1
        sprintf(work_string,"Bilinear Filtering [%s], Mipmapping [%s], Mip Distance [%5.2f]",((bilinear_mode == 1) ? "ON" : "OFF"),
                                                                                          ((mip_mode == 1) ? "ON" : "OFF"),
                                                                                          mipdistance);
        ...
        if (help_mode==1)
        {
            ...
            draw_text("<B>..............Toggle Bilinear Filtering.", 0, text_y+=12, RGB(255,255,255));
            draw_text("<M>..............Toggle Mip Mapping.", 0, text_y+=12, RGB(255,255,255));
            draw_text("<1>, <2>.........Decrease/Increase Mipdistance.", 0, text_y+=12, RGB(255,255,255));
        ...
        draw_text(work_string, 0, WINDOW_HEIGHT-34-32, RGB(0,255,0));

        sprintf(work_string,"CAM [%5.2f, %5.2f, %5.2f]",  cam.pos.x, cam.pos.y, cam.pos.z);
        draw_text(work_string, 0, WINDOW_HEIGHT-34-48, RGB(0,255,0));
