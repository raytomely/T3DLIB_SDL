// DEMOII14_5.CPP - shadow demo with planar projective shadows
// on actual lightsource position with fan object for comparison
// to lightmapping
// READ THIS!
...
int Game_Init(void *parms)
{
    ...
    VECTOR4D_INITXYZ(&vscale, 10, 10, 10);
    ...
    for (int index_obj=0; index_obj < NUM_OBJECTS; index_obj++)
    {
        ...
        obj_array[index_obj].world_pos.y = 150;
...
int Game_Main(void *parms)
{
    ...
    while(main_loop)
    {
        ...
        // generate rotation matrix around y axis
        Build_XYZ_Rotation_MATRIX4X4(0, 2.0f, 0, &mrot);
        ...
        for (int vertex=0; vertex < obj_work->num_vertices; vertex++)
        {
            ...
            obj_work->vlist_trans[vertex].v.y = 10.0; // pl.y + t0*(vi.y - pl.y);
        ...
        if (help_mode==1)
        {
            ...
            draw_text("<3>,<4>..........Change height of red point light.", 0, text_y+=12, RGB(255,255,255));
