// DEMOII13_4.CPP - BSP Demo with z buffering
// READ THIS!
...
OBJECT4DV2      obj_scene,     // floor/ceiling object
                obj_moveable;  // the movable object
...
int Game_Init(void *parms)
{
    ...
    mouse_init(MOUSE_ABSOLUTE_MODE);
    ...
    // load the sounds
    beep0_sound_id = load_wav("menuitem.wav");
        ...
        switch(editor_state)
        {
              ...
              case EDITOR_STATE_EDIT: /////////////////////////////////////////////////
              {
                    ...
                    if (texture_index_wall >= 0)
                    {
                       ...
                       Draw_Scaled_BOB16(&textures, 64,64, screen);
                       ...
                       if (nearest_line >= 0)
                       {
                          ...
                          Draw_Scaled_BOB16(&textures, 32,32, screen);

                          // now draw height of line
                          sprintf(buffer,"%d",lines[nearest_line].height);
                          draw_text(buffer, 750,188,RGB(0,255,0));
                    ...
                    if (texture_index_wall >= 0)
                    {
                       ...
                       draw_text(buffer, 738,336-56,RGB(0,255,0));
                    } // end if
                    else
                    {
                       draw_text("None", 738,336,RGB(0,255,0));
                    } // end

                    // wall height
                    sprintf(buffer,"%d",wall_height);
                    draw_text(buffer, 688, 188,RGB(0,255,0));

                    // texture index for floors
                    if (texture_index_floor >= 0)
                    {
                       sprintf(buffer,"%d",texture_index_floor);
                       draw_text(buffer, 738,488,RGB(0,255,0));
                    } // end if
                    else
                    {
                       draw_text("None", 738,488,RGB(0,255,0));
                    } // end
                    ...
                    draw_text(buffer, 8,WINDOW_HEIGHT - 18,RGB(0,255,0));

                    draw_text("<V>...View mode.", WINDOW_WIDTH - 265, WINDOW_HEIGHT - 18, RGB(255,255,255));
              ...
              case EDITOR_STATE_VIEW: ///////////////////////////////////////////////
              {
                    ...
                    //////////////////////////////////////////////////////////////////////////////
                    // begin pass one -- with all static objects with writethru on the Z buffer...
                    //////////////////////////////////////////////////////////////////////////////
                    ...
                    //////////////////////////////////////////////////////////////////////
                    // begin pass two -- with all moveable objects with normal z buffer...
                    //////////////////////////////////////////////////////////////////////
                    ...
                    // reset the object (this only matters for backface and object removal)
                    Reset_OBJECT4DV2(&obj_moveable);

                    static float rec_count = 0;

                    // set position of object
                    obj_moveable.world_pos.x = 500*Fast_Cos(rec_count);
                    obj_moveable.world_pos.y = 40 + 20*Fast_Sin(rec_count*2);
                    obj_moveable.world_pos.z = 500*Fast_Sin(rec_count);

                    if ((rec_count+=.5) > 360)
                       rec_count =- 360;

                    // attempt to cull object
                    if (!Cull_OBJECT4DV2(&obj_moveable, &cam, CULL_OBJECT_XYZ_PLANES))
                    {
                       // create an identity to work with
                       MAT_IDENTITY_4X4(&mrot);
                /*
                       // now align recognizer with circular path by creating a coordinate
                       // system such that Z is aligned with the direction of travel
                       // or we can use the rotation function to do it for us :)
                       mrot.M[0][0] = Fast_Cos(rec_count);
                       mrot.M[0][1] = 0;
                       mrot.M[0][2] = Fast_Sin(rec_count);

                       mrot.M[1][0] = 0;
                       mrot.M[1][1] = 1;
                       mrot.M[1][2] = 0;

                       mrot.M[2][0] = -Fast_Sin(rec_count);
                       mrot.M[2][1] = 0;
                       mrot.M[2][2] = Fast_Cos(rec_count);
                */
                       // generate rotation matrix around y axis
                       Build_XYZ_Rotation_MATRIX4X4(0, -rec_count, 0, &mrot);

                       // rotate the local coords of the object
                       Transform_OBJECT4DV2(&obj_moveable, &mrot, TRANSFORM_LOCAL_TO_TRANS,1);

                       // perform world transform
                       Model_To_World_OBJECT4DV2(&obj_moveable, TRANSFORM_TRANS_ONLY);

                       // insert the object into render list
                       Insert_OBJECT4DV2_RENDERLIST4DV2(&rend_list, &obj_moveable,0);
                    } // end if
                ...
                */
                    ...
                    // draw instructions
                    draw_text("Press ESC to return to editor mode. Press <H> for Help.", 0, 0, RGB(0,255,0));
                    ...
                    if (help_mode==1)
                    {
                        ...
                        draw_text("<P>..............Toggle point lights.", 0, text_y+=12, RGB(255,255,255));
                        draw_text("<B>..............Toggle backface removal.", 0, text_y+=12, RGB(255,255,255));
                        draw_text("<S>..............Toggle Z sorting.", 0, text_y+=12, RGB(255,255,255));
                        draw_text("<C>..............Toggle BSP Backface Culling.", 0, text_y+=12, RGB(255,255,255));
                        draw_text("<Z>..............Toggle Z buffering.", 0, text_y+=12, RGB(255,255,255));
                        draw_text("<Z>..............Toggle Wireframe mode.", 0, text_y+=12, RGB(255,255,255));
    ...
    for (int cell_y = 0; cell_y < BSP_CELLS_Y-1; cell_y++)
    {
        for (int cell_x = 0; cell_x < BSP_CELLS_X-1; cell_x++)
        {
            ...
            if (texture_id > -1)
            {
               ...
               Draw_Scaled_BOB16(&textures, BSP_GRID_SIZE,BSP_GRID_SIZE, screen); //lpdds);
    ...
    for (int index = 0; index < NUM_GADGETS; index++)
    {
        ...
        switch(curr_button->type)
        {
              case GADGET_TYPE_TOGGLE:    // toggles from off/on, on/off
              {
                   ...
                   // draw the button
                   Draw_BOB16(&button_img, screen); //lpdds);
...
int Load_BSP_File_LEV(char *filename)
{
    ...
    if (version!=1.0f)
    {
       ...
       //MessageBox(main_window_handle, buffer, "BSP Level Generator",MB_OK);
       fprintf(stderr, "Error - Wrong Version. %s \n",filename);
       return(0);
    } // end if
    ...
    if (num_sections!=2)
    {
       ...
       //MessageBox(main_window_handle, buffer, "BSP Level Generator",MB_OK);
       fprintf(stderr, "Error - Wrong Number of Sections. %s \n",filename);
       return(0);
    } // end if
