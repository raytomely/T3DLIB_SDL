                            ...
                            int lpitch)           // memory pitch of buffer
{
    ...
    for (int index = 0; index < num_bindings; index++)
    {
        ...
        for (sindex = 1; sindex < num_segments; sindex++)
        {
...
int Load_Bitmap_Font(char *fontfile, BOB_PTR font)
{
    ...
    // hardware blitter
    Create_BOB(font, 0,0,16,14,64, BOB_ATTR_VISIBLE | BOB_ATTR_SINGLE_FRAME, 0, 0, 16);
...
int Game_Init(void *parms)
{
    ...
    game_over_id       = load_wav("gameover.wav");
    ambient_systems_id = load_wav("stationthrob2.wav");
    ...
    Create_BOB(&starscape, 0,0,800,600,1, BOB_ATTR_VISIBLE | BOB_ATTR_SINGLE_FRAME, 0, 0, 16);
    ...
    Create_BOB(&crosshair, 0,0,CROSS_WIDTH, CROSS_HEIGHT,1, BOB_ATTR_VISIBLE | BOB_ATTR_SINGLE_FRAME, 0, 0, 16);
...
int Game_Main(void *parms)
{
    ...
    while(main_loop)
    {
        ...
        case GAME_STATE_RESTART:
        {
            ...
            play_sound(ambient_systems_id, -1);
        ...
        case GAME_STATE_OVER: // keep running sim, but kill diff_level, and player input
        {
            ...
            blit_surface(starscape.images[starscape.curr_frame], NULL, screen, NULL);
            ...
            // draw cockpit
            Draw_BOB16(&cockpit, screen);//lpddsback);

            // draw information
            sprintf(work_string, "SCORE:%d", score);
            Draw_Bitmap_Font_String(&tech_font, TPOS_SCORE_X,TPOS_SCORE_Y, work_string, FONT_HPITCH, FONT_VPITCH, screen);//lpddsback);

            sprintf(work_string, "HITS:%d", hits);
            Draw_Bitmap_Font_String(&tech_font, TPOS_HITS_X,TPOS_HITS_Y, work_string, FONT_HPITCH, FONT_VPITCH, screen);//lpddsback);

            sprintf(work_string, "ESCAPED:%d", escaped);
            Draw_Bitmap_Font_String(&tech_font, TPOS_ESCAPED_X,TPOS_ESCAPED_Y, work_string, FONT_HPITCH, FONT_VPITCH, screen);//lpddsback);

            sprintf(work_string, "LEVEL:%2.4f", diff_level);
            Draw_Bitmap_Font_String(&tech_font, TPOS_GLEVEL_X,TPOS_GLEVEL_Y, work_string, FONT_HPITCH, FONT_VPITCH, screen);//lpddsback);

            sprintf(work_string, "VEL:%3.2fK/S", player_z_vel+diff_level);
            Draw_Bitmap_Font_String(&tech_font, TPOS_SPEED_X,TPOS_SPEED_Y, work_string, FONT_HPITCH, FONT_VPITCH, screen);//lpddsback);

            sprintf(work_string, "NRG:%d", weapon_energy);
            Draw_Bitmap_Font_String(&tech_font, TPOS_ENERGY_X,TPOS_ENERGY_Y, work_string, FONT_HPITCH, FONT_VPITCH, screen);//lpddsback);
            ...
            if (game_state == GAME_STATE_OVER)
            {
                ...
                Draw_Bitmap_Font_String(&tech_font, TPOS_GINFO_X-(FONT_HPITCH/2)*strlen(work_string),TPOS_GINFO_Y, work_string, FONT_HPITCH, FONT_VPITCH, screen);//lpddsback);

                sprintf(work_string, "PRESS ENTER TO PLAY AGAIN");
                Draw_Bitmap_Font_String(&tech_font, TPOS_GINFO_X-(FONT_HPITCH/2)*strlen(work_string),TPOS_GINFO_Y + 2*FONT_VPITCH, work_string, FONT_HPITCH, FONT_VPITCH, screen);//lpddsback);

                // logic...
                if (keystate[KBD_RETURN])
                {
                    game_state = GAME_STATE_RESTART;
                } // end if
            ...
            if (game_state == GAME_STATE_RUN)
            {
                ...
                Draw_BOB16(&crosshair, screen);//lpddsback);
            } // end if
